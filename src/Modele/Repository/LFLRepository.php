...
class LFLRepository extends RepositoryMere
{
    public static function getAllJoueurs(): bool|array
    {
        $sql = "SELECT * from joueurs where Dle = 'LFL' AND active = 1";
    ...
    {
        $sql = "SELECT nbPersonnes from nbReussites WHERE Dle = 'lfldle' AND date = '" .date("Y-m-d") ."'";
    ...
    {
        $sql = "SELECT * FROM joueurs where ID = (select ID from joueurdujour WHERE date = '" .date("Y-m-d") ."') AND Dle = 'LFL'";
...
//    {
//        $sql = "SELECT * from joueurs where Nom = :nom AND Dle = 'LFL'";
//        $tab = array(":nom" => $nom);
//
...
//        {
//            if($fake === "nofake"){
//                $sql = "UPDATE `nbReussites` SET `nbPersonnes`= nbPersonnes+1 WHERE dle = 'lfldle' AND date = CURRENT_DATE ";
